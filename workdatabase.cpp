...
WorkDatabase::WorkDatabase(QObject *parent) : QObject(parent)
{
    ...
    if(base.open()) {
        ...
        for(int i = 0; query->next(); i++) {
           list_ProjectTableModel.append(query->record().value(0).toString());
        }
    }
    else {
        qDebug() << "base error";
...
void WorkDatabase::updateTableModel(QString name) {
...
QStringList WorkDatabase::getlist_ProjectTableModel() {
    return list_ProjectTableModel;
}
