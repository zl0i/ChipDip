...
WebElement::WebElement(QObject *parent) : QThread(parent)
{
    ...
    if(periodUpdate == 0) setPeriodUpdate(7);
    uint lastUpdate = setting.value("LastUpdate").toUInt();
    if(lastUpdate == 0) SaveNowDataTime();

    currentShop = setting.value("shop/name").toString();

    query->exec("select town, town_id from ChipDip_town_id;"); //проверка наличия таблицы
    if(!query->lastError().isValid()) { // если есть считываем и заполняем хэш
        for(int i = 0; query->next(); i++) {
            town_id[query->value(0).toString()] = query->value(1).toString();
        }
    }
    else {                                      // если нету, то создаем, заполянем таблицу и хэш
        fillTableChipDip();
    ...
    if(currentShop.size() == 0) {      //если нету сохранения, то сохраняем первый магазин
...
void WebElement::ChekUpdateTime() {
    uint LastTimeUpdate = setting.value("LastUpdate").toUInt();
    periodUpdate = setting.value("PeriodUpdate").toUInt();
...
void WebElement::SaveNowDataTime() {
    setting.setValue("LastUpdate", QDateTime::currentSecsSinceEpoch());
}
...
void WebElement::fillTableChipDip() {
    if(!query->exec("create table ChipDip_town_id (id  INTEGER PRIMARY KEY, "
                                                "town CHAR(50) NOT NULL UNIQUE,"
                                                "town_id CHAR(50) NOT NULL UNIQUE);")) {
